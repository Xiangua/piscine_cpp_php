...
int main() {
	...
	int				lvlId = 0;
	Level			*lvls[NB_LVL];
	lvls[0] = new Level ("Level 1 - Asteroid field", -5 );
	lvls[1] = new Level ("Level 2 - Asteroid field", -4 );
	lvls[2] = new Level ("Level 3 - Asteroid field", -2 );
	...
	while (running)
	{
		...
		BoxText->fixeDialog(lvls[lvlId]->name, 5, 1);
		timer.stop();
		timer.wait();
	}
	delete BoxHead;
