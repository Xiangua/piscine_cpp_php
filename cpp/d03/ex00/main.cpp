...
void	attack(FragTrap & p1, FragTrap & p2)
{
	int rand1 = rand() % 3;
	unsigned int nb_dmg;
...
int		 main()
{
	...
	FragTrap *p2 = new FragTrap("player2");
	bool live1 = true;
	bool live2 = true;
	srand(time(NULL));
	...
	if (live1)
	{
		std::cout << "le Gagnant est : " << p1->getName() << std::endl;
		delete p1;
	}
	else if (live2)
	{
		std::cout << "le Gagnant est : " << p2->getName() << std::endl;
		delete p2;
	}
	return 0;
}
